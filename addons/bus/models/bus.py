...
import zira
from zira import api, fields, models, SUPERUSER_ID
from zira.tools.misc import DEFAULT_SERVER_DATETIME_FORMAT
from zira.tools import date_utils
...
class ImBus(models.Model):
    ...
    def sendmany(self, notifications):
        ...
        if channels:
            ...
            def notify():
                with zira.sql_db.db_connect('postgres').cursor() as cr:
...
class ImDispatch(object):
    ...
    def poll(self, dbname, channels, last, options=None, timeout=TIMEOUT):
        ...
        if not zira.evented:
            ...
            current.setName("ziraerp.longpolling.request.%s" % current.ident)
...
dispatch = None
if not zira.multi_process or zira.evented:
