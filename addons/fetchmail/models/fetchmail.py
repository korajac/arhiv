...
class FetchmailServer(models.Model):
    ...
    script = fields.Char(readonly=True, default='/mail/static/scripts/ziraerp_mailgate.py')
    ...
    def onchange_server_type(self):
        ...
        self.configuration = """
            ...
            ziraerp_mailgate.py --host=HOSTNAME --port=PORT -u %(uid)d -p PASSWORD -d %(dbname)s
            Example configuration for the postfix mta running locally:
            /etc/postfix/virtual_aliases:
            @youdomain ziraerp_mailgate@localhost
            /etc/aliases:
            ziraerp_mailgate: "|/path/to/ziraerp-mailgate.py --host=localhost -u %(uid)d -p PASSWORD -d %(dbname)s"
        """ % conf
