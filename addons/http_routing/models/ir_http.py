...
from zira.addons.base.models.ir_http import RequestUID, ModelConverter
from zira.http import request
from zira.tools import config, ustr, pycompat
...
class IrHttp(models.AbstractModel):
    ...
    def _geoip_setup_resolver(cls):
        ...
        if zira._geoip_resolver is not None:
            return
        geofile = config.get('geoip_database')
        try:
            zira._geoip_resolver = GeoIPResolver.open(geofile) or False
    ...
    def _geoip_resolve(cls):
        if 'geoip' not in request.session:
            record = {}
            if zira._geoip_resolver and request.httprequest.remote_addr:
                record = zira._geoip_resolver.resolve(request.httprequest.remote_addr) or {}
    ...
    def _postprocess_args(cls, arguments, rule):
        ...
        except zira.exceptions.MissingError:
