...
class StatusController(http.Controller):
    ...
    def connect_box(self, token):
        """
        This route is called when we want that a IoT Box will be connected to a Zira DB
        token is a base 64 encoded string and have 2 argument separate by |
        1 - url of zira DB
        2 - token. This token will be compared to the token of Zira. He have 1 hour lifetime
        """
...
class ConnectionManager(Thread):
    ...
    def run(self):
        if not helpers.get_zira_server_url():
    ...
    def _connect_box(self):
        ...
        req = requests.post('https://iot-proxy.korajac.com/zira-enterprise/iot/connect-box', json=data, verify=False)
...
class Manager(Thread):
    ...
    def load_drivers(self):
        """
        This method loads local files: 'zira/addons/hw_drivers/drivers'
    ...
    def send_alldevices(self):
        """
        This method send IoT Box and devices informations to Zira database
        """
        server = helpers.get_zira_server_url()
        if server:
            subject = helpers.read_file_first_line('zira-subject.conf')
            if subject:
            ...
            _logger.warning('Zira server not set')
    ...
    def run(self):
        """
        Thread that will check connected/disconnected device, load drivers if needed and contact the zira server with the updates
        """
