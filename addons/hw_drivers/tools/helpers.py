...
class IoTRestart(Thread):
    """
    Thread to restart zira server in IoT Box when we must return a answer before
    """
...
def add_credential(db_uuid, enterprise_code):
    write_file('zira-db-uuid.conf', db_uuid)
    write_file('zira-enterprise-code.conf', enterprise_code)

def check_certificate():
    """
    Check if the current certificate is up to date or not authenticated
    """
    server = get_zira_server_url()
    if server:
        ...
        if path.exists():
            with path.open('r') as f:
                ...
                if cn == 'ZiraTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch():
    """
    Check if the local branch is the same than the connected Zira DB and
    ...
    if server:
        ...
        try:
            ...
            if response.status == 200:
                git = ['git', '--work-tree=/home/pi/zira/', '--git-dir=/home/pi/zira/.git']
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/zira/addons/hw_drivers/drivers/*"])
...
def check_image():
    ...
    """
    url = 'http://nightly.korajac.com/master/posbox/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def get_zira_server_url():
    ap = subprocess.call(['systemctl', 'is-active', '--quiet', 'hostapd']) # if service is active return 0 else inactive
    if not ap:
        return False
    return read_file_first_line('zira-remote-server.conf')
...
def load_certificate():
    """
    Send a request to Zira with customer db_uuid and enterprise_code to get a true certificate
    """
    db_uuid = read_file_first_line('zira-db-uuid.conf')
    enterprise_code = read_file_first_line('zira-enterprise-code.conf')
    if db_uuid and enterprise_code:
        url = 'https://www.korajac.com/zira-enterprise/iot/x509'
        data = {
            'params': {
                'db_uuid': db_uuid,
        ...
        if result:
            write_file('zira-subject.conf', result['subject_cn'])
            subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
...
def download_drivers(auto=True):
    """
    Get the drivers from the configured Zira server
    """
    server = get_zira_server_url()
    if server:
        urllib3.disable_warnings()
        ...
        try:
            ...
            if resp.data:
                subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'zira/addons/hw_drivers/drivers'
