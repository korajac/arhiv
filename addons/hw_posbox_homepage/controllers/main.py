...
import os
from zira.tools import misc
from pathlib import Path
import threading

from uuid import getnode as get_mac
from zira.addons.hw_proxy.controllers import main as hw_proxy
from zira.addons.web.controllers import main as web
from zira.modules.module import get_resource_path
from zira.addons.hw_drivers.tools import helpers
from zira.addons.hw_drivers.controllers.driver import iot_devices
from zira.http import Response
...
if hasattr(sys, 'frozen'):
    ...
    loader = jinja2.PackageLoader('zira.addons.hw_posbox_homepage', "views")
...
class IoTboxHomepage(web.Home):
    ...
    def clean_partition(self):
        subprocess.check_call(['sudo', 'bash', '-c', '. /home/pi/zira/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; cleanup'])
    ...
    def get_homepage_data(self):
        ...
        return {
            ...
            'server_status': helpers.get_zira_server_url() or 'Not Configured',
    ...
    def index(self):
        ...
        remote_server = Path.home() / 'zira-remote-server.conf'
    ...
    def list_credential(self):
        return list_credential_template.render({
            'title': "Zira's IoT Box - List credential",
            'breadcrumb': 'List credential',
            'db_uuid': helpers.read_file_first_line('zira-db-uuid.conf'),
            'enterprise_code': helpers.read_file_first_line('zira-enterprise-code.conf'),
        })
    ...
    def clear_credential(self):
        helpers.unlink_file('zira-db-uuid.conf')
        helpers.unlink_file('zira-enterprise-code.conf')
        subprocess.check_call(["sudo", "service", "zira", "restart"])
        return "<meta http-equiv='refresh' content='20; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_wifi(self, essid, password, persistent=False):
        ...
        if server:
            res_payload['server'] = {
                'url': server,
                'message': 'Redirect to Zira Server'
            }
        else:
            res_payload['server'] = {
    ...
    def clear_server_configuration(self):
        helpers.unlink_file('zira-remote-server.conf')
        return "<meta http-equiv='refresh' content='0; url=http://" + helpers.get_ip() + ":8069'>"
    ...
    def connect_to_server(self, token, iotname):
        if token:
            ...
            url = helpers.get_zira_server_url()
    ...
    def step_by_step_configure_page(self):
        return configure_wizard_template.render({
            ...
            'ssid': helpers.get_wifi_essid(),
            'server': helpers.get_zira_server_url() or '',
    ...
    def server(self):
        return server_config_template.render({
            'title': 'IoT -> Zira server configuration',
            'breadcrumb': 'Configure Zira Server',
            'hostname': subprocess.check_output('hostname').decode('utf-8').strip('\n'),
            'server_status': helpers.get_zira_server_url() or 'Not configured yet',
    ...
    def upgrade(self):
        commit = subprocess.check_output(["git", "--work-tree=/home/pi/zira/", "--git-dir=/home/pi/zira/.git", "log", "-1"]).decode('utf-8').replace("\n", "<br/>")
        ...
        return upgrade_page_template.render({
            'title': "Zira's IoTBox - Software Upgrade",
    ...
    def perform_upgrade(self):
        ...
        os.system('/home/pi/zira/addons/point_of_sale/tools/posbox/configuration/posbox_update.sh')
    ...
    def perform_flashing_create_partition(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/zira/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; create_partition']).decode().split('\n')[-2]
    ...
    def perform_flashing_download_raspbian(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/zira/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; download_raspbian']).decode().split('\n')[-2]
    ...
    def perform_flashing_copy_raspbian(self):
        try:
            response = subprocess.check_output(['sudo', 'bash', '-c', '. /home/pi/zira/addons/point_of_sale/tools/posbox/configuration/upgrade.sh; copy_raspbian']).decode().split('\n')[-2]
