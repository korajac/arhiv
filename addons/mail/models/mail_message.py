...
from ziraerp.http import request

from zira import _, api, fields, models, modules, SUPERUSER_ID, tools
from zira.exceptions import UserError, AccessError
from zira.osv import expression
from zira.tools import groupby, formataddr
...
class Message(models.Model):
    ...
    def _notify_compute_recipients(self, record, msg_vals):
        ...
        for pid, cid, active, pshare, ctype, notif, groups in res:
            ...
            if pid:
                if active is False:
                    # avoid to notify inactive partner by email (zirabot)
                    continue
