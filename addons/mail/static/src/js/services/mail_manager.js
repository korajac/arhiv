zira.define('mail.Manager', function (require) {
"use strict";
...
var MailManager =  AbstractService.extend({
    ...
    _ZIRABOT_ID: "ZIRABOT", // default authorID for transient messages
    ...
    /**
     * Get the ZiraBot ID, which is the default authorID for transient messages
     *
     * @returns {string}
     */
    getZirabotID: function () {
        return this._ZIRABOT_ID;
    },
    /**
            ...
            if (thread) {
                if (
                    ...
                    if (thread.isTwoUserThread() && options.showNotification) {
                        ...
                        if (!self.call('bus_service', 'isZiraFocused') || !query.isVisible) {
    ...
    _initializeInternalState: function () {
        ...
        // # of message received when zira is out of focus
    ...
    _notifyIncomingMessage: function (message) {
        if (this.call('bus_service', 'isZiraFocused')) {
            // no need to notify
            return;
        }
