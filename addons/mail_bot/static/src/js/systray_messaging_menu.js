zira.define('mail_bot.systray.MessagingMenu', function (require) {
"use strict";
...
return MessagingMenu.include({
     ...
     * messaging menu (e.g. 'ZiraBot has a request')
     *
     * @override
     * @private
     ...
     * ZiraBot, and the ZiraBot influences the counter by 1 when it requests
    ...
    _handleResponseNotificationPermission: function (value) {
        ...
        if (value !== 'granted') {
            this.call('bus_service', 'sendNotification', _t('Permission denied'),
                _t('Zira will not have the permission to send native notifications on this device.'));
        } else {
            this.call('bus_service', 'sendNotification', _t('Yay, push notifications are enabled!'),
                _t('Zira has now the permission to send you native notifications on this device.'));
        }
        this._updateCounter();
    },
    /**
    ...
    /**
     * Override so that it handles preview related to ZiraBot
     *
     * @override
     * @private
     * @param {MouseEvent} ev
