zira.define('partner.autocomplete.core', function (require) {
'use strict';
...
return {
    _dropPreviousZira: new concurrency.DropPrevious(),
    ...
    autocomplete: function (value) {
        ...
        var ziraPromise = this._getZiraSuggestions(value, isVAT).then(function (suggestions){
            ziraSuggestions = suggestions;
        });
        ...
        var concatResults = function () {
            // Add Clearbit result with Zira result (with unique domain)
            if (clearbitSuggestions && clearbitSuggestions.length) {
                var websites = ziraSuggestions.map(function (suggestion) {
                ...
                clearbitSuggestions.forEach(function (suggestion) {
                    if (websites.indexOf(suggestion.domain) < 0) {
                        ...
                        ziraSuggestions.push(suggestion);
                    }
                });
            }

            ziraSuggestions = _.filter(ziraSuggestions, function (suggestion) {
                return !suggestion.ignored;
            });
            _.each(ziraSuggestions, function(suggestion){
              delete suggestion.ignored;
            });
            return def.resolve(ziraSuggestions);
        };

        this._whenAll([ziraPromise, clearbitPromise]).then(concatResults, concatResults);
    ...
    /**
     * Use Zira Autocomplete API to return suggestions
     *
     * @param {string} value
     * @param {boolean} isVAT
     * @returns {Deferred}
     * @private
     */
    _getZiraSuggestions: function (value, isVAT) {
        ...
        return this._dropPreviousZira.add(def);
    },
    /**
