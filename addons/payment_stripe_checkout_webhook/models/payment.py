...
from zira.tools.float_utils import float_round
from zira.tools import consteq
...
class PaymentAcquirerStripeCheckoutWH(models.Model):
    ...
    stripe_webhook_secret = fields.Char(
        ...
        help="If you enable webhooks, this secret is used to verify the electronic "
             "signature of events sent by Stripe to Zira. Failing to set this field in Zira "
    ...
    def _verify_stripe_signature(self):
        ...
        if not consteq(expected_signature, actual_signature):
            _logger.error(
                ...
                'in Zira matches to one in the Stripe dashboard')
    ...
    def _handle_checkout_webhook(self, checkout_object):
        ...
        try:
            zira_tx = self.env['payment.transaction']._stripe_form_get_tx_from_data(data)
        except ValidationError as e:
            _logger.info('Received notification for tx %s. Skipped it because of %s', tx_reference, e)
            return False

        PaymentAcquirerStripeCheckoutWH._verify_stripe_signature(zira_tx.acquirer_id)

        url = 'payment_intents/%s' % zira_tx.stripe_payment_intent
        stripe_tx = zira_tx.acquirer_id._stripe_request(url)
        ...
        if 'error' in stripe_tx:
            ...
            raise ValidationError("Could not fetch Stripe payment intent related to %s because of %s; see %s" % (
                zira_tx, error['message'], error['doc_url']))
        ...
        return zira_tx.form_feedback(data, 'stripe')
