...
echo "addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/zira" >> .git/info/sparse-checkout

git fetch "${localremote}" "${localbranch}" --depth=1
git reset "${localremote}"/"${localbranch}" --hard

git clean -df
cp -a /home/pi/zira/addons/point_of_sale/tools/posbox/overwrite_after_init/home/pi/zira/* /home/pi/zira/
rm -r /home/pi/zira/addons/point_of_sale/tools/posbox/overwrite_after_init
...
(sleep 5 && sudo service zira restart) &
