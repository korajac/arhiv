...
from zira import fields as zira_fields, tools, _
from zira.exceptions import ValidationError, AccessError, MissingError, UserError
from zira.http import content_disposition, Controller, request, route
...
class CustomerPortal(Controller):
    ...
    def _get_archive_groups(self, model, domain=None, fields=None, groupby="create_date", order="create_date desc"):
        ...
        for group in request.env[model]._read_group_raw(domain, fields=fields, groupby=groupby, orderby=order):
            ...
            groups.append({
                'date_begin': zira_fields.Date.to_string(zira_fields.Date.from_string(date_begin)),
                'date_end': zira_fields.Date.to_string(zira_fields.Date.from_string(date_end)),
                'name': label,
