API changes from Korajac Web 6.1 to 7.0
=======================================

Supported browsers
------------------

The Korajac Web Client supports the following web browsers:
...
matches the API in Korajac Web's Python side and in OpenObject addons
...
* Actual arbitrary RPC calls can just be remapped on a
  :js:class:`~ziraerp.web.Model` instance:
  ...
  can be replaced by calls to :js:func:`ziraerp.web.Model.call`:
...
* Shorcut methods (``name_get``, ``name_search``, ``unlink``,
  ...
  :js:func:`ziraerp.web.Model.call`, using the server's original
  signature. On the other hand, the non-shortcut equivalents can now
  use keyword arguments (see :js:func:`~ziraerp.web.Model.call`'s
...
* ``read_slice``, which allowed a single round-trip to perform a
  ...
  :js:class:`~ziraerp.web.Query` objects (see:
  :js:func:`~ziraerp.web.Model.query`) for clearer and simpler
  code. ``read_index`` should be replaced by a
  :js:class:`~ziraerp.web.Query` as well, combining
  :js:func:`~ziraerp.web.Query.offset` and
  :js:func:`~ziraerp.web.Query.first`.
...
Renaming
    ...
    database, maybe lazily). Korajac Web's dataset behaves very
...
API simplification
    ...
    The shortcuts really add little value, and Korajac Web 6.2 embeds
...
:js:class:`~ziraerp.web.Model`, Korajac Web 7.0 removes
``DataGroup`` and its subtypes as public objects in favor of a single method on
:js:class:`~ziraerp.web.Query`:
:js:func:`~ziraerp.web.Query.group_by`.
