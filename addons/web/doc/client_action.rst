...
Client actions are the client-side version of Korajac's "Server
...
In the Korajac Web code, a client action only requires two pieces of
information:
...
* Providing said object. Two different types of objects can be mapped
  ...
  * An Korajac Web widget, which must inherit from
    :js:class:`ziraerp.web.Widget`
...
* if, on the other hand, the client action maps to a
  :js:class:`~ziraerp.web.Widget`, that
  :js:class:`~ziraerp.web.Widget` will be instantiated and added to
  the web client's canvas, with the usual
  :js:class:`~ziraerp.web.Widget` lifecycle (essentially, it will
...
object::
    ...
    // Registers the object 'ziraerp.web_dashboard.Widget' to the client
...
At this point, the generic :js:class:`~ziraerp.web.Widget` lifecycle
...
should be performed in the :js:func:`~ziraerp.web.Widget.start()`
method.

.. note::

    As required by :js:class:`~ziraerp.web.Widget`'s contract, if
    :js:func:`~ziraerp.web.Widget.start()` executes any asynchronous
