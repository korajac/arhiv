...
A very basic Korajac module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An Korajac
...
.. note::
    ...
    restart the Korajac server itself for it to be taken in account.
...
provides: an instance of the Korajac Web client, with the contents of
...
its :js:func:`~ziraerp.web.Widget.start` to add some content to its
DOM:
...
will require restarting the Korajac Server to see the changes, as
usual):
...
.. note::
    ...
    Prefixing CSS rules with both ``.ziraerp`` (to ensure the rule
    will apply only within the confines of the Korajac Web client) and
...
be done in :js:func:`~ziraerp.web.Widget.start` but that gets unwieldy
...
Korajac Web's template language is :doc:`qweb`. Although any
...
integration to Korajac Web widgets.
...
To prevent this irremediable loss, we can use Korajac's support for
...
:js:func:`~ziraerp.base.Widget.start()` is to perform *asynchronous*
...
recorded previously using the :js:class:`~ziraerp.web.Query` interface
...
4. And finally — after restarting Korajac — navigating to the test
...
side-effect of this is to :js:func:`~ziraerp.web.Widget.start` it,
...
:js:func:`set a widget's DOM <ziraerp.web.Widget.setElement>`, let's
