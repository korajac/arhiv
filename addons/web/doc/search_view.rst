...
Korajac Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (Korajac
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the Korajac Web 7
...
The primary component of search views, as with all other Korajac
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from Korajac field types (and widgets) to search view
objects is stored in the ``ziraerp.web.search.fields``
:js:class:`~ziraerp.web.Registry` where new field types and widgets
can be added.
...
:js:func:`~ziraerp.web.search.Input.facet_for_defaults` on each input,
passing it a mapping (a javascript object) of ``name:value`` extracted
from the action's context.

This method should fetch a :js:class:`~ziraerp.web.search.Facet` (or
...
:js:func:`ziraerp.web.search.Input.facet_for` with that value.

There is no default implementation of
:js:func:`ziraerp.web.search.Input.facet_for` [#no_impl]_, but
:js:class:`ziraerp.web.search.Field` provides one, which uses the
value as-is to fetch a :js:class:`~ziraerp.web.search.Facet`.
...
inputs through the :js:func:`~ziraerp.web.search.Input.complete`
method.
...
``facet``
    ...
    Either a :js:class:`~ziraerp.web.search.Facet` object or (more
...
.. note::
    ...
    <ziraerp.web.search.Input.visible>`, its completion function will
    *not* be called.
...
:js:func:`~ziraerp.web.search.Input.in_drawer`.

By default, :js:func:`~ziraerp.web.search.Input.in_drawer` returns the
value of :js:attr:`~ziraerp.web.search.Input._in_drawer`, which is
``false``. The behavior can be toggled either by redefining the
attribute to ``true`` (either on the class or on the input), or by
overriding :js:func:`~ziraerp.web.search.Input.in_drawer` itself.
...
.. note::
    ...
    An :js:func:`invisible <ziraerp.web.search.Input.visible>` input
...
Korajac this is done via :ref:`domains <ziraerpserver:domains>`. On
the other hand, the Korajac Web 7 search view's state is modelled
after a collection of :js:class:`~ziraerp.web.search.Facet`, and each
...
:js:class:`~ziraerp.web.search.Facet` objects to Korajac search data.

This is done via an input's
:js:func:`~ziraerp.web.search.Input.get_domain` and
:js:func:`~ziraerp.web.search.Input.get_context`. Each takes a
:js:class:`~ziraerp.web.search.Facet` and returns whatever it's
...
context, and can throw an :js:class:`~ziraerp.web.search.Invalid`
exception if the value is not valid at all for the field.

.. note::

    The :js:class:`~ziraerp.web.search.Facet` object can have any
    number of values (from 1 upwards)

.. note::

    There is a third conversion method,
    :js:func:`~ziraerp.web.search.Input.get_groupby`, which returns an
...
:js:class:`~ziraerp.web.search.SearchQuery` available as
:js:attr:`ziraerp.web.SearchView.query`.

The query is a `backbone collection`_ of
:js:class:`~ziraerp.web.search.Facet` objects, which can be interacted
with directly by external objects or search view controls
(e.g. widgets displayed in the drawer).

.. js:class:: ziraerp.web.search.SearchQuery

    The current search query of the search view, provides convenience
    behaviors for manipulating :js:class:`~ziraerp.web.search.Facet`
    on top of the usual `backbone collection`_ methods.

    The query ensures all of its facets contain at least one
    :js:class:`~ziraerp.web.search.FacetValue` instance. Otherwise,
    the facet is automatically removed from the query.

    .. js:function:: ziraerp.web.search.SearchQuery.add(values, options)
    ...
    .. js:function:: ziraerp.web.search.SearchQuery.toggle(value, options)
...
.. js:class:: ziraerp.web.search.Facet
    ...
    .. js:attribute:: field
        ...
        The :js:class:`~ziraerp.web.search.Input` instance which
    ...
    .. js:attribute:: values
        ...
        :js:class:`~ziraerp.web.search.FacetValues` as a javascript
...
.. js:class:: ziraerp.web.search.FacetValues

    `Backbone collection`_ of
    :js:class:`~ziraerp.web.search.FacetValue` instances.

.. js:class:: ziraerp.web.search.FacetValue
    ...
    .. js:attribute:: value
        ...
        :js:class:`~ziraerp.web.search.Input` to serialize to domains
        and contexts.
...
:js:class:`~ziraerp.web.search.Field` provides a default
implementation of :js:func:`~ziraerp.web.search.Input.get_domain` and
:js:func:`~ziraerp.web.search.Input.get_context` taking care of most
of the peculiarities pertaining to Korajac's handling of fields in
...
:js:func:`~ziraerp.web.search.Input.get_domain` or
:js:func:`~ziraerp.web.search.Input.get_context`:

.. js:function:: ziraerp.web.search.Field.get_context(facet)

    If the field has no ``@context``, simply returns
    ``null``. Otherwise, calls
    :js:func:`~ziraerp.web.search.Field.value_from` once for each
    :js:class:`~ziraerp.web.search.FacetValue` of the current
    :js:class:`~ziraerp.web.search.Facet` (in order to extract the
    basic javascript object from the
    :js:class:`~ziraerp.web.search.FacetValue` then evaluates
    ``@context`` with each of these values set as ``self``, and
    returns the union of all these contexts.

    :param facet:
    :type facet: ziraerp.web.search.Facet
    :returns: a context (literal or compound)

.. js:function:: ziraerp.web.search.Field.get_domain(facet)

    If the field has no ``@filter_domain``, calls
    :js:func:`~ziraerp.web.search.Field.make_domain` once with each
    :js:class:`~ziraerp.web.search.FacetValue` of the current
    :js:class:`~ziraerp.web.search.Facet` as well as the field's
    ``@name`` and either its ``@operator`` or
    :js:attr:`~ziraerp.web.search.Field.default_operator`.

    If the field has an ``@filter_value``, calls
    :js:func:`~ziraerp.web.search.Field.value_from` once per
    :js:class:`~ziraerp.web.search.FacetValue` and evaluates
    ``@filter_value`` with each of these values set as ``self``.

    In either case, "ors" all of the resulting domains (using ``|``)
    if there is more than one
    :js:class:`~ziraerp.web.search.FacetValue` and returns the union
    of the result.
...
.. js:function:: ziraerp.web.search.Field.make_domain(name, operator, facetValue)

    Builds a literal domain from the provided data. Calls
    :js:func:`~ziraerp.web.search.Field.value_from` on the
    :js:class:`~ziraerp.web.search.FacetValue` and evaluates and sets
    ...
    :param facetValue:
    :type facetValue: ziraerp.web.search.FacetValue
    :returns: Array<(String, String, Object)>

.. js:function:: ziraerp.web.search.Field.value_from(facetValue)

    Extracts a "bare" javascript value from the provided
    :js:class:`~ziraerp.web.search.FacetValue`, and returns it.
...
.. js:attribute:: ziraerp.web.search.Field.default_operator

    Operator used to build a domain when a field has no ``@operator``
    or ``@filter_domain``. ``"="`` for
    :js:class:`~ziraerp.web.search.Field`

Arbitrary data storage
----------------------

:js:class:`~ziraerp.web.search.Facet` and
:js:class:`~ziraerp.web.search.FacetValue` objects (and structures)
...
Korajac Web 6.1 to Korajac Web 7.
...
* :js:func:`ziraerp.web.SearchView.do_clear` has been removed
* :js:func:`ziraerp.web.SearchView.do_toggle_filter` has been removed

Widgets API
+++++++++++

* :js:func:`ziraerp.web.search.Widget.render` has been removed

* :js:func:`ziraerp.web.search.Widget.make_id` has been removed

* Search field objects are not ziraerp widgets anymore, their
  ``start`` is not generally called

* :js:func:`~ziraerp.web.search.Input.clear` has been removed since
  clearing the search view now simply consists of removing all search
  facets

* :js:func:`~ziraerp.web.search.Input.get_domain` and
  :js:func:`~ziraerp.web.search.Input.get_context` now take a
  :js:class:`~ziraerp.web.search.Facet` as parameter, from which it's
  their job to get whatever value they want

* :js:func:`~ziraerp.web.search.Input.get_groupby` has been added. It returns
  an :js:class:`Array` of context-like constructs. By default, it does not do
  anything in :js:class:`~ziraerp.web.search.Field` and it returns the various
  contexts of its enabled filters in
  :js:class:`~ziraerp.web.search.FilterGroup`.

Filters
+++++++

* :js:func:`ziraerp.web.search.Filter.is_enabled` has been removed

* :js:class:`~ziraerp.web.search.FilterGroup` instances are still
...
* ``get_value`` has been replaced by
  :js:func:`~ziraerp.web.search.Field.value_from` as it now takes a
  :js:class:`~ziraerp.web.search.FacetValue` argument (instead of no
  argument). It provides a default implementation returning the
  ``value`` property of its argument.

* The third argument to
  :js:func:`~ziraerp.web.search.Field.make_domain` is now a
  :js:class:`~ziraerp.web.search.FacetValue` so child classes have all
...
* Because the autocompletion service is now provided by the search
  view itself,
  :js:func:`ziraerp.web.search.ManyToOneField.setup_autocomplete` has
  been removed.
...
* The advanced search is now a more standard
  :js:class:`~ziraerp.web.search.Input` configured to be rendered in
  the drawer.

* :js:class:`~ziraerp.web.search.ExtendedSearchProposition.Field` are
  now standard widgets, with the "right" behaviors (they don't rebind
  their ``$element`` in ``start()``)

* The ad-hoc optional setting of the ziraerp field descriptor on a
  :js:class:`~ziraerp.web.search.ExtendedSearchProposition.Field` has
  been removed, the field descriptor is now passed as second argument
  to the
  :js:class:`~ziraerp.web.search.ExtendedSearchProposition.Field`'s
  constructor, and bound to its
  :js:attr:`~ziraerp.web.search.ExtendedSearchProposition.Field.field`.

* Instead of its former domain triplet ``(field, operator, value)``,
  :js:func:`~ziraerp.web.search.ExtendedSearchProposition.get_proposition`
...
.. [#facet-field]
    ...
    :js:class:`~ziraerp.web.search.Input`, nor does it need to be what
    created the facet, it just needs to provide the three
    facet-serialization methods
    :js:func:`~ziraerp.web.search.Input.get_domain`,
    :js:func:`~ziraerp.web.search.Input.get_context` and
    :js:func:`~ziraerp.web.search.Input.get_gropuby`, existing
    :js:class:`~ziraerp.web.search.Input` subtypes merely provide
    ...
    :js:class:`~ziraerp.web.search.Input`, as
    :js:class:`~ziraerp.web.search.CustomFilters` and
    :js:class:`~ziraerp.web.search.Advanced` do.
