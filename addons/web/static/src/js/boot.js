...
(function() {
    ...
    var zira = window.zira = window.zira || {};
    _.extend(zira, {
            ...
            if (jobs.length) {
                ...
                for (var k=0; k<jobs.length; k++) {
                    debug_jobs[jobs[k].name] = job = {
                        ...
                        dependents: zira.__DEBUG__.get_dependents(jobs[k].name),
                        name: jobs[k].name
                    };
                    if (jobs[k].error) {
                    ...
                    var deps = zira.__DEBUG__.get_dependencies( job.name );
                ...
                missing = zira.__DEBUG__.get_missing_jobs();
                failed = zira.__DEBUG__.get_failed_jobs();
                var unloaded = _.filter(debug_jobs, function (job) { return job.missing; });

                if (zira.debug || !_.isEmpty(failed) || !_.isEmpty(unloaded)) {
                    ...
                    if (zira.debug && !_.isEmpty(debug_jobs)) {
                        log('Debug:                   ', debug_jobs);
                    }
                }
            }
            zira.__DEBUG__.js_modules = {
                missing: missing,
                failed: _.pluck(failed, 'name'),
            };
            zira.__DEBUG__.didLogInfo.resolve();
        },
                ...
                if (!job.error) {
                    $.when(job_exec).then(
                        function (data) {
                            ...
                            def.resolve();
                            zira.process_jobs(jobs, services);
                        }, function (e) {
                            job.rejected = e || true;
                            jobs.push(job);
                            def.resolve();
