zira.define('web.mixins', function (require) {
"use strict";
...
function ZiraEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
ZiraEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};

ZiraEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
var EventDispatcherMixin = _.extend({}, ParentedMixin, {
     ...
     * Korajac Web where developers may want to replace existing callbacks with
     * theirs.
     *
    ...
    trigger_up: function (name, info) {
        var event = new ZiraEvent(this, name, info);
