zira.define('web.Session', function (require) {
"use strict";
...
var Session = core.Class.extend(mixins.EventDispatcherMixin, {
    ...
    init: function (parent, origin, options) {
        ...
        options = options || {};
        this.module_list = (options.modules && options.modules.slice()) || (window.zira._modules && window.zira._modules.slice()) || [];
        this.server = null;
    ...
    on_modules_loaded: function () {
        ...
        for(var j=0; j<this.module_list.length; j++) {
            ...
            ziraerp[mod] = {};
            // init module mod
            var fct = ziraerp._ziraerp[mod];
            if(typeof(fct) === "function") {
                ziraerp._ziraerp[mod] = {};
                for (var k in fct) {
                    ziraerp._ziraerp[mod][k] = fct[k];
                }
                fct(ziraerp, ziraerp._ziraerp[mod]);
            }
    ...
    session_reload: function () {
        var result = _.extend({}, window.zira.session_info);
        delete result.session_id;
