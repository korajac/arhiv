...
from zira.addons.website.tools import MockRequest


@zira.tests.tagged('post_install', '-at_install')
class TestUi(zira.tests.HttpCase):
    def test_01_admin_shop_tour(self):
        self.phantom_js("/", "zira.__DEBUG__.services['web_tour.tour'].run('shop')", "zira.__DEBUG__.services['web_tour.tour'].tours.shop.ready", login="admin")

    def test_02_admin_checkout(self):
        self.phantom_js("/", "zira.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "zira.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="admin")

    def test_03_demo_checkout(self):
        self.phantom_js("/", "zira.__DEBUG__.services['web_tour.tour'].run('shop_buy_product')", "zira.__DEBUG__.services['web_tour.tour'].tours.shop_buy_product.ready", login="demo")


@zira.tests.tagged('post_install', '-at_install')
class TestWebsiteSaleCheckoutAddress(zira.tests.TransactionCase):
