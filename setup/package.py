...
exec(open(os.path.join(ROOTDIR, 'zira', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/zira && \
    groupadd -g %(group_id)s zira && \
    useradd -u %(user_id)s -g zira zira -d /var/lib/zira && \
    mkdir /data && \
    chown zira:zira /var/lib/zira /data
USER zira
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class ZiraTestTimeoutError(Exception):
    pass


class ZiraTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise ZiraTestError("Installation of package failed")
        else:
        ...
        raise ZiraTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/zira_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False):
    ...
    run_cmd(cmd + ['%s/' % args.zira_dir, args.build_dir])
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'zira/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'zira-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
    ...
    def run(self, cmd, build_dir, container_name, user='zira', exposed_port=None, detach=False, timeout=None):
    ...
    def test_zira(self):
        logging.info('Starting to test Zira install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)
            if os.path.exists(os.path.join(args.build_dir, 'zira.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise ZiraTestTimeoutError('Zira pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise ZiraTestError('Error while installing/starting Zira after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'zira-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/zira-*.tar.gz' % self.args.build_dir)[0], '%s/zira_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/zira-*.zip' % self.args.build_dir)[0], '%s/zira_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'pip3 install /data/src/zira_%s.%s.tar.gz' % (VERSION, TSTAMP),
            'su postgres -s /bin/bash -c "createuser -s zira"',
            'su postgres -s /bin/bash -c "createdb mycompany"',
            'su zira -s /bin/bash -c "zira -d mycompany -i base --stop-after-init"',
            'su zira -s /bin/bash -c "zira -d mycompany --pidfile=/data/src/zira.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'zira-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_zira()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/zira (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../zira_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'zira-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            '/usr/bin/dpkg -i /data/src/zira_%s.%s_all.deb ; /usr/bin/apt-get install -f -y' % (VERSION, TSTAMP),
            'su zira -s /bin/bash -c "zira -d mycompany -i base --stop-after-init"',
            'su zira -s /bin/bash -c "zira -d mycompany --pidfile=/data/src/zira.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'zira-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_zira()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py --quiet bdist_rpm', self.args.build_dir, 'zira-rpm-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/zira-*.noarch.rpm' % self.args.build_dir)[0], '%s/zira_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'dnf install -d 0 -e 0 /data/src/zira_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su zira -s /bin/bash -c "zira -c /etc/zira/zira.conf -d mycompany -i base --stop-after-init"',
            'su zira -s /bin/bash -c "zira -c /etc/zira/zira.conf -d mycompany --pidfile=/data/src/zira.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'zira-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_zira()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'zira-rpm-createrepo-%s' % TSTAMP)
...
class KVMWinBuildExe(KVM):
    def run(self):
        ...
        remote_build_dir = '/cygdrive/c/zirabuild/server/'

        self.ssh("mkdir -p build")
        logging.info("Syncing Zira files to virtual machine...")
...
class KVMWinTestExe(KVM):
    def run(self):
        ...
        setup_path = glob("%s/ziraerp-server-setup-*.exe" % self.args.build_dir)[0]
        setupfile = setup_path.split('/')[-1]
        setupversion = setupfile.split('ziraerp-server-setup-')[1].split('.exe')[0]

        self.rsync(['"%s"' % setup_path, '%s@127.0.0.1:' % self.login])
        self.ssh("TEMP=/tmp ./%s /S" % setupfile)
        self.ssh('PGPASSWORD=openpgpwd /cygdrive/c/"Program Files"/"Zira %s"/PostgreSQL/bin/createdb.exe -e -U openpg mycompany' % setupversion)
        self.ssh('netsh advfirewall set publicprofile state off')
        self.ssh('/cygdrive/c/"Program Files"/"Zira {sv}"/python/python.exe \'c:\\Program Files\\Zira {sv}\\server\\zira-bin\' -d mycompany -i base --stop-after-init'.format(sv=setupversion))
...
def build_exe(args):
    ...
    shutil.copy(glob('%s/ziraerp*.exe' % args.build_dir)[0], '%s/zira_%s.%s.exe' % (args.build_dir, VERSION, TSTAMP))
...
def parse_args():
    ...
    # Windows VM
    ap.add_argument("--vm-winxp-image", default='/home/zira/vm/win1036/win10_winpy36.qcow2', help="%(default)s")
    ap.add_argument("--vm-winxp-ssh-key", default='/home/zira/vm/win1036/id_rsa', help="%(default)s")
