...
!define PUBLISHER 'Ražanica d.o.o.'
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\Zira Server ${VERSION}"
...
OutFile "ziraerp-allinone-setup-${VERSION}.exe"
...
InstallDir "$PROGRAMFILES\Zira ${VERSION}"
...
#VIAddVersionKey "LegalTrademark" "Korajac is a trademark of ${PUBLISHER}"
...
!define MUI_ICON "${PIXMAPS_PATH}\ziraerp-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\ziraerp-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\ziraerp-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\ziraerp-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.korajac.com/page/contactus"
...
; English
LangString DESC_Korajac_Server ${LANG_ENGLISH} "Install the Zira Server with all the Zira standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by Zira."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact Zira for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the Zira Server"
...
LangString TITLE_Korajac_Server ${LANG_ENGLISH} "Zira Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start Zira"

; French
LangString DESC_Korajac_Server ${LANG_FRENCH} "Installation du Serveur Zira avec tous les modules Zira standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par Zira."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez Zira pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur Zira"
...
LangString TITLE_Korajac_Server ${LANG_FRENCH} "Serveur Zira"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer Zira"

InstType $(Profile_AllInOne)
InstType $(Profile_Server)

Section $(TITLE_Korajac_Server) SectionKorajac_Server
    SectionIn 1 2
...
# If there is a previous install of the Korajac Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "addons_path" "$INSTDIR\server\zira\addons"
    WriteIniStr "$INSTDIR\server\zira.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\zira.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\zira-bin" --stop-after-init --logfile "$INSTDIR\server\zira.log" -s'
    ${If} ${RunningX64}
      nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe" "\"$INSTDIR\server\zira-bin\""'
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\${POSTGRESQL_EXE_FILENAME}" \
        ...
        --servicename "PostgreSQL_For_Zira" \
...
;    WriteRegDWORD HKLM     "${UNINSTALL_REGISTRY_KEY}" "VersionMinor" "${REVISION_VERSION}"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@korajac.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+387.49.206.209"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.korajac.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@korajac.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${SectionKorajac_Server} $(DESC_Korajac_Server)
...
Function ShowPostgreSQL
    SectionGetFlags ${SectionKorajac_Server} $0
