...
import zira
from zira import api, fields, models, modules, tools, _
from zira.addons.base.models.ir_model import MODULE_UNINSTALL_FLAG
from zira.exceptions import AccessDenied, UserError
from zira.osv import expression
from zira.tools.parse_version import parse_version
from zira.tools.misc import topological_sort
...
class Module(models.Model):
    ...
    license = fields.Selection([
        ...
        ('OEEL-1', 'Zira Enterprise Edition License v1.0'),
        ('OPL-1', 'Zira Proprietary License v1.0'),
    ...
    to_buy = fields.Boolean('Zira Enterprise Module', default=False)
    ...
    def install_from_urls(self, urls):
        ...
        KORAJAC = zira.release.product_name.lower()
        ...
        try:
            ...
            for module_name, url in urls.items():
                ...
                try:
                    _logger.info('Downloading module `%s` from Korajac Apps', module_name)
            ...
            for module_name, url in urls.items():
                if module_name == KORAJAC or not url:
                    continue    # KORAJAC is special case, handled below, and no URL means local module
            ...
            if urls.get(KORAJAC):
                ...
                # copy all modules in the SERVER/zira/addons directory to the new "zira" module (except base itself)
                for d in os.listdir(base_path):
                    if d != 'base' and os.path.isdir(os.path.join(base_path, d)):
                        destdir = os.path.join(tmp, KORAJAC, 'addons', d)    # XXX 'zira' subdirectory ?
                ...
                _logger.info('Copy downloaded module `zira` to `%s`', server_dir)
                shutil.move(os.path.join(tmp, KORAJAC), server_dir)
                #if bck:
                #    shutil.rmtree(bck)
    ...
    def get_apps_server(self):
        return tools.config.get('apps_server', 'https://apps.korajac.com/apps')
