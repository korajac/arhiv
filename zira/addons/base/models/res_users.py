...
from zira import api, fields, models, tools, SUPERUSER_ID, _
from zira.exceptions import AccessDenied, AccessError, UserError, ValidationError
from zira.http import request
from zira.osv import expression
from zira.service.db import check_super
from zira.tools import partition, pycompat, collections
...
class Users(models.Model):
    """ User class. A res.users record models an Korajac user and is different
        from an employee.
    ...
    def unlink(self):
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Zira (updates, module installation, ...)'))
        db = self._cr.dbname
        for id in self.ids:
            self.__uid_cache[db].pop(id, None)
        self._invalidate_session_cache()
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: zira.exceptions.AccessDenied when old password is wrong
        :raise: zira.exceptions.UserError when new password is not set or empty
        """
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warn(
                    ...
                    "and *might* be a proxy. If your Zira is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Zira in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.korajac.com/documentation/12.0/administration/deployment/deploy.html#https for details.",
                    source
                )
