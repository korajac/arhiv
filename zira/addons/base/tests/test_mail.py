...
from zira.tests.common import SavepointCase
from zira.tools import html_sanitize, append_content_to_html, plaintext2html, email_split, misc, decode_smtp_header
...
class EmailConfigCase(SavepointCase):
    @patch.dict("zira.tools.config.options", {"email_from": "settings@example.com"})
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'zira.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@zira.example.com')
        set_param('mail.catchall.domain', 'zira.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@zira.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@zira.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@zira.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@zira.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@zira.example.com>')
        set_param('mail.catchall.domain', 'zira.example.com')
