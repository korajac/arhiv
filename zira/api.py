...
from zira.tools import frozendict, classproperty, StackMap, pycompat
from zira.exceptions import CacheMiss
...
def constrains(*args):
    ...
    Should raise :class:`~zira.exceptions.ValidationError` if the
    validation failed.
...
def one(method):
    """ Decorate a record-style method where ``self`` is expected to be a
        ...
        .. deprecated:: 9.0
            ...
            is a single record with :meth:`~zira.models.Model.ensure_one`.
    """
...
from zira.exceptions import UserError, AccessError, MissingError
