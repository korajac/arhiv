...
"""
Korajac - Server
Korajac is an ERP+CRM program for small and medium businesses.

The whole source code is distributed under the terms of the
GNU Public Licence.

(c) 2003-TODAY, Fabien Pinckaers - Korajac SA
"""
...
__author__ = zira.release.author
__version__ = zira.release.version

# Also use the `zira` logger for the main script.
_logger = logging.getLogger('zira')
...
def report_configuration():
    ...
    """
    config = zira.tools.config
    _logger.info("Zira version %s", __version__)
    if os.path.isfile(config.rcfile):
        _logger.info("Using configuration file at " + config.rcfile)
    _logger.info('addons paths: %s', zira.modules.module.ad_paths)
...
def setup_pid_file():
    ...
    """
    config = zira.tools.config
    if not zira.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    with open(config["translate_out"], "wb") as buf:
        registry = zira.modules.registry.Registry.new(dbname)
        with zira.api.Environment.manage():
            with registry.cursor() as cr:
                zira.tools.trans_export(config["language"],
...
def import_translation():
    ...
    with zira.api.Environment.manage():
        with registry.cursor() as cr:
            zira.tools.trans_load(
                cr, config["translate_in"], config["language"], context=context,
            )

def main(args):
    check_root_user()
    zira.tools.config.parse_config(args)
    check_postgres_user()
    ...
    if config['db_name']:
        ...
        for db_name in preload:
            try:
                zira.service.db._create_empty_database(db_name)
                ...
                if err.pgcode == errorcodes.INSUFFICIENT_PRIVILEGE:
                    ...
                    raise err
            except zira.service.db.DatabaseExists:
                pass
    ...
    if config['workers']:
        zira.multi_process = True

    stop = config["stop_after_init"]

    setup_pid_file()
    rc = zira.service.server.start(preload=preload, stop=stop)
    sys.exit(rc)

class Server(Command):
    """Start the zira server (default command)"""
    def run(self, args):
        main(args)
