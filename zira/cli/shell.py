...
class Shell(Command):
    """Start zira in an interactive shell"""
    supported_shells = ['ipython', 'ptpython', 'bpython', 'python']

    def init(self, args):
        config.parse_config(args)
        zira.cli.server.report_configuration()
        zira.service.server.start(preload=[], stop=True)
        ...
        with zira.api.Environment.manage():
            if dbname:
                ...
                with registry.cursor() as cr:
                    ...
                    ctx = zira.api.Environment(cr, uid, {})['res.users'].context_get()
                    env = zira.api.Environment(cr, uid, ctx)
                    local_vars['env'] = env
