...
# *broken*
NO_POSTMORTEM = (zira.osv.orm.except_orm,
                 zira.exceptions.AccessError,
                 zira.exceptions.ValidationError,
                 zira.exceptions.MissingError,
                 zira.exceptions.AccessDenied,
                 zira.exceptions.Warning,
                 zira.exceptions.RedirectWarning)
    ...
    try:
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_request and rpc_response_flag:
                zira.netsvc.log(rpc_request, logging.DEBUG, '%s.%s' % (service_name, method), replace_request_password(params))

        threading.current_thread().uid = None
        threading.current_thread().dbname = None
        if service_name == 'common':
            dispatch = zira.service.common.dispatch
        elif service_name == 'db':
            dispatch = zira.service.db.dispatch
        elif service_name == 'object':
            dispatch = zira.service.model.dispatch
        ...
        if rpc_request_flag or rpc_response_flag:
            ...
            if rpc_response_flag:
                zira.netsvc.log(rpc_response, logging.DEBUG, logline, result)
            else:
                zira.netsvc.log(rpc_request, logging.DEBUG, logline, replace_request_password(params), depth=1)

        return result
    except NO_POSTMORTEM:
        raise
    except zira.exceptions.DeferredException as e:
        _logger.exception(zira.tools.exception_to_unicode(e))
        zira.tools.debugger.post_mortem(zira.tools.config, e.traceback)
        raise
    except Exception as e:
        _logger.exception(zira.tools.exception_to_unicode(e))
        zira.tools.debugger.post_mortem(zira.tools.config, sys.exc_info())
        raise
...
class WebRequest(object):
    """ Parent class for all Zira Web request types, mostly deals with
    ...
    def __init__(self, httprequest):
        ...
        # dispatching phase in zira.service.wsgi_server.application
        if self.db:
    ...
    @property
    def cr(self):
        """ :class:`~zira.sql_db.Cursor` initialized for the current method call.
    ...
    @property
    def env(self):
        """ The :class:`~zira.api.Environment` bound to current request. """
        if self._env is None:
            self._env = zira.api.Environment(self.cr, self.uid, self.context)
        return self._env
    ...
    @lazy_property
    def session(self):
        """ :class:`KorajacSession` holding the HTTP session data for the
    ...
    def _handle_exception(self, exception):
        ...
        if not isinstance(exception, NO_POSTMORTEM) \
            ...
            zira.tools.debugger.post_mortem(
                zira.tools.config, sys.exc_info())
...
def route(route=None, **kw):
    ...
    .. admonition:: CSRF Protection
        ...
        Zira implements token-based `CSRF protection
        ...
        * if the form is generated in Python, a csrf token is
          ...
          <zira.http.WebRequest.csrf_token`, the
          :data:`~zira.http.request` object is available by default
        ...
        * if the endpoint can be called by external parties (not from
          Zira) as e.g. it is a REST API or a `webhook
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (zira.exceptions.Warning, SessionExpiredException,
                                          zira.exceptions.except_orm, werkzeug.exceptions.NotFound)):
                _logger.exception("Exception during JSON request handling.")
            error = {
                    'code': 200,
                    'message': "Zira Server Error",
            ...
            if isinstance(exception, AuthenticationError):
                error['code'] = 100
                error['message'] = "Zira Session Invalid"
            if isinstance(exception, SessionExpiredException):
                error['code'] = 100
                error['message'] = "Zira Session Expired"
...
def serialize_exception(e):
    ...
    if isinstance(e, zira.exceptions.UserError):
        tmp["exception_type"] = "user_error"
    elif isinstance(e, zira.exceptions.Warning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, zira.exceptions.RedirectWarning):
        tmp["exception_type"] = "warning"
    elif isinstance(e, zira.exceptions.AccessError):
        tmp["exception_type"] = "access_error"
    elif isinstance(e, zira.exceptions.MissingError):
        tmp["exception_type"] = "missing_error"
    elif isinstance(e, zira.exceptions.AccessDenied):
        tmp["exception_type"] = "access_denied"
    elif isinstance(e, zira.exceptions.ValidationError):
        tmp["exception_type"] = "validation_error"
    elif isinstance(e, zira.exceptions.except_orm):
...
Zira URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.korajac.com/documentation/12.0/developer/reference/http.html#csrf for
more details.

* if this endpoint is accessed through Zira via py-QWeb form, embed a CSRF
...
class ControllerType(type):
    def __init__(cls, name, bases, attrs):
        ...
        if not class_path[:2] == ["zira", "addons"]:
            module = ""
        else:
...
def routing_map(modules, nodb_only, converters=None):
    ...
    def get_subclasses(klass):
        def valid(c):
            return c.__module__.startswith('zira.addons.') and c.__module__.split(".")[2] in modules
...
class KorajacSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(KorajacSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def authenticate(self, db, login=None, password=None, uid=None):
        ...
        if uid is None:
            ...
            uid = zira.registry(db)['res.users'].authenticate(db, login, password, env)
        else:
    ...
    def check_security(self):
        ...
        env = zira.api.Environment(request.cr, self.uid, self.context)
    ...
    def _fix_lang(self, context):
        """ Korajac provides languages which may not make sense and/or may not
...
class Response(werkzeug.wrappers.Response):
    ...
    def render(self):
        ...
        """
        env = request.env(user=self.uid or request.uid or zira.SUPERUSER_ID)
...
class Root(object):
    """Root WSGI application for the Korajac Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        # Setup http sessions
        path = zira.tools.config.session_dir
        _logger.debug('HTTP sessions stored in: %s', path)
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=KorajacSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + zira.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in zira.modules.module.ad_paths:
    ...
    def dispatch(self, environ, start_response):
        ...
        try:
            ...
            with request:
                ...
                if db:
                    try:
                        zira.registry(db).check_signaling()
                        with zira.tools.mute_logger('zira.sql_db'):
...
def db_list(force=False, httprequest=None):
    dbs = zira.service.db.list_dbs(force)
...
def db_filter(dbs, httprequest=None):
    ...
    if d == "www" and r:
        d = r.partition('.')[0]
    if zira.tools.config['dbfilter']:
        d, h = re.escape(d), re.escape(h)
        r = zira.tools.config['dbfilter'].replace('%h', h).replace('%d', d)
        dbs = [i for i in dbs if re.match(r, i)]
    elif zira.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Zira will
        # use the value of --database as a comma seperated list of exposed databases.
        exposed_dbs = set(db.strip() for db in zira.tools.config['db_name'].split(','))
...
def send_file(filepath_or_fp, mimetype=None, as_attachment=False, filename=None, mtime=None,
    ...
    if isinstance(mtime, str):
        try:
            server_format = zira.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT
...
def content_disposition(filename):
    filename = zira.tools.ustr(filename)
