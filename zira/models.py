...
class MetaModel(api.Meta):
    ...
    def _get_addon_name(self, full_name):
        # The (Korajac) module name can be in the ``zira.addons`` namespace
        # or not. For instance, module ``sale`` can be imported as
        # ``zira.addons.sale`` (the right way) or ``sale`` (for backward
        # compatibility).
        module_parts = full_name.split('.')
        if len(module_parts) > 2 and module_parts[:2] == ['zira', 'addons']:
...
class BaseModel(MetaModel('DummyModel', (object,), {'_register': False})):
    """ Base class for Zira models.

    Zira models are created by inheriting:
    ...
    *   :class:`AbstractModel` for abstract super classes meant to be shared by
    ...
    def compute_concurrency_field(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = zira.fields.Datetime.now()

    @api.depends('create_date', 'write_date')
    def compute_concurrency_field_with_access(self):
        for record in self:
            record[self.CONCURRENCY_CHECK_FIELD] = \
                record.write_date or record.create_date or zira.fields.Datetime.now()
    ...
    def _add_fake_fields(self, fields):
        from zira.fields import Char, Integer
    ...
    def write(self, vals):
        ...
        * For numeric fields (:class:`~zira.fields.Integer`,
          :class:`~zira.fields.Float`) the value should be of the
          corresponding type
        * For :class:`~zira.fields.Boolean`, the value should be a
          :class:`python:bool`
        * For :class:`~zira.fields.Selection`, the value should match the
          selection values (generally :class:`python:str`, sometimes
          :class:`python:int`)
        * For :class:`~zira.fields.Many2one`, the value should be the
        ...
        * Other non-relational fields use a string for value
          ...
          .. danger::
              ...
              :class:`~zira.fields.Date` and
              :class:`~zira.fields.Datetime` fields use strings as values
              (written and read) rather than :class:`~python:datetime.date` or
              :class:`~python:datetime.datetime`. These date strings are
              UTC-only and formatted according to
              :const:`zira.tools.misc.DEFAULT_SERVER_DATE_FORMAT` and
              :const:`zira.tools.misc.DEFAULT_SERVER_DATETIME_FORMAT`
        * .. _ziraerp/models/relationals/format:

          :class:`~zira.fields.One2many` and
          :class:`~zira.fields.Many2many` use a special "commands" format to
    ...
    def _where_calc(self, domain, active_test=True):
        """Computes the WHERE clause needed to implement an Korajac domain.
    ...
    def with_env(self, env):
        ...
        :type env: :class:`~zira.api.Environment`
        """
...
class Model(AbstractModel):
    """ Main super-class for regular database-persisted Zira models.

    Zira models are created by inheriting from this class::
