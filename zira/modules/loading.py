...
import zira
import zira.modules.db
import zira.modules.graph
import zira.modules.migration
import zira.modules.registry
import zira.tools as tools

from zira import api, SUPERUSER_ID
from zira.modules.module import adapt_version, initialize_sys_path, load_ziraerp_module

_logger = logging.getLogger(__name__)
_test_logger = logging.getLogger('zira.tests')
...
def force_demo(cr):
    ...
    """
    graph = zira.modules.graph.Graph()
...
def load_module_graph(cr, graph, status=None, perform_checks=True,
    ...
    def load_test(idref, mode):
        ...
        try:
            ...
            zira.registry(cr.dbname).clear_caches()
    ...
    migrations = zira.modules.migration.MigrationManager(cr, graph)
    ...
    for index, package in enumerate(graph, 1):
        ...
        load_ziraerp_module(package.name)

        new_install = package.state == 'to install'
        if new_install:
            py_module = sys.modules['zira.addons.%s' % (module_name,)]
        ...
        if needs_update:
            ...
            overwrite = zira.tools.config["overwrite_existing_translations"]
            ...
            if demo_loaded:
                ...
                if tools.config.options['test_enable']:
                    ...
                    report.record_result(zira.modules.module.run_unit_tests(module_name, cr.dbname))
    ...
    _logger.log(25, "%s modules loaded in %.2fs, %s queries", len(graph), time.time() - t0, zira.sql_db.sql_counter - t0_sql)
...
def load_modules(db, force_demo=False, status=None, update_module=False):
    ...
    with db.cursor() as cr:
        if not zira.modules.db.is_initialized(cr):
        ...
        # zira.modules.registry.Registry.new().
        registry = zira.registry(cr.dbname)
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['zira.addons.%s' % (pkg.name,)]
                ...
                registry = zira.modules.registry.Registry.new(
