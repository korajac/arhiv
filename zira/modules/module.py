...
import zira
import zira.tools as tools
import zira.release as release
from zira import SUPERUSER_ID, api
from zira.tools import pycompat

MANIFEST_NAMES = ('__manifest__.py', '__ziraerp__.py')
...
class AddonsHook(object):
    """ Makes modules accessible through ziraerp.addons.* and zira.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('zira.addons.', 'ziraerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        # get canonical names
        zira_name = re.sub(r'^ziraerp.addons.(\w+)$', r'zira.addons.\g<1>', name)
        ziraerp_name = re.sub(r'^zira.addons.(\w+)$', r'ziraerp.addons.\g<1>', zira_name)

        assert zira_name not in sys.modules
        assert ziraerp_name not in sys.modules
        ...
        # create empty zira.addons.* module, set name
        new_mod = types.ModuleType(zira_name)
        new_mod.__loader__ = self

        # module top-level can only be a package
        assert type_ == imp.PKG_DIRECTORY, "Zira addon top-level must be a package"
        modfile = opj(path, '__init__.py')
        new_mod.__file__ = modfile
        new_mod.__path__ = [path]
        new_mod.__package__ = zira_name

        # both base and alias should be in sys.modules to handle recursive and
        # corecursive situations
        sys.modules[zira_name] = sys.modules[ziraerp_name] = new_mod

        # execute source in context of module *after* putting everything in
        # sys.modules, so recursive import works
        exec(open(modfile, 'rb').read(), new_mod.__dict__)

        # people import ziraerp.addons and expect ziraerp.addons.<module> to work
        setattr(zira.addons, addon_name, new_mod)
...
class ZiraHook(object):
    """ Makes zira package also available as ziraerp """

    def find_module(self, name, path=None):
        # ziraerp.addons.<identifier> should already be matched by AddonsHook,
        # only framework and subdirectories of modules should match
        if re.match(r'^ziraerp\b', name):
            return self

    def load_module(self, name):
        assert name not in sys.modules

        canonical = re.sub(r'^ziraerp(.*)', r'zira\g<1>', name)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import Korajac addons from the different
    addons paths.

    This ensures something like ``import crm`` (or even
    ``import zira.addons.crm``) works even if the addons are not in the
    PYTHONPATH.
    """
    global ad_paths
    global hooked
    ...
    # add zira.addons.__path__
    for ad in __import__('zira.addons').addons.__path__:
        ad = os.path.abspath(ad)
        if ad not in ad_paths and os.path.isdir(ad):
            ad_paths.append(ad)

    if not hooked:
        sys.meta_path.insert(0, ZiraHook())
...
def get_module_filetree(module, dir='.'):
    ...
    files = zira.tools.osutil.listdir(path, True)
...
def load_ziraerp_module(module_name):
    """ Load an Korajac module, if not already loaded.
    ...
    initialize_sys_path()
    try:
        __import__('zira.addons.' + module_name)
        ...
        if info['post_load']:
            getattr(sys.modules['zira.addons.' + module_name], info['post_load'])()
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('zira.addons', module)

    try:
        importlib.import_module('zira.addons.base.maintenance.migrations.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('zira.addons.base.maintenance.migrations', module)
...
class TestStream(object):
    def __init__(self, logger_name='zira.tests'):
...
def run_unit_tests(module_name, dbname, position='at_install'):
    ...
    from zira.tests.common import TagsSelector, ZiraSuite
    ...
    for m in mods:
        ...
        suite = ZiraSuite(t for t in tests if position_tag.check(t) and config_tags.check(t))
        ...
        if suite.countTestCases():
            ...
            if time.time() - t0 > 5:
                _logger.log(25, "%s tested in %.2fs, %s queries", m.__name__, time.time() - t0, zira.sql_db.sql_counter - t0_sql)
