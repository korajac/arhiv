...
class PostgreSQLHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with tools.ignore(Exception), tools.mute_logger('zira.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    # Check that handler.stream has a fileno() method: when running Korajac
...
DEFAULT_LOG_CONFIGURATION = [
    'zira.http.rpc.request:INFO',
    'zira.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['zira:DEBUG', 'zira.sql_db:INFO', 'zira.http.rpc:DEBUG'],
    'debug_rpc': ['zira:DEBUG', 'zira.sql_db:INFO', 'zira.http.rpc.request:DEBUG'],
    'debug': ['zira:DEBUG', 'zira.sql_db:INFO'],
    'debug_sql': ['zira.sql_db:DEBUG'],
    'info': [],
    'warn': ['zira:WARNING', 'werkzeug:WARNING'],
    'error': ['zira:ERROR', 'werkzeug:ERROR'],
    'critical': ['zira:CRITICAL', 'werkzeug:CRITICAL'],
}
