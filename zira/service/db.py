...
import zira
from zira import SUPERUSER_ID
from zira.exceptions import AccessDenied
import zira.release
import zira.sql_db
import zira.tools
from zira.sql_db import db_connect
from zira.release import version_info
...
def check_db_management_enabled(method):
    def if_db_mgt_enabled(method, self, *args, **kwargs):
        if not zira.tools.config['list_db']:
...
def check_super(passwd):
    if passwd and zira.tools.config.verify_admin_password(passwd):
        return True
    raise zira.exceptions.AccessDenied()

# This should be moved to zira.modules.db, along side initialize().
def _initialize_db(id, db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        db = zira.sql_db.db_connect(db_name)
        with closing(db.cursor()) as cr:
            # TODO this should be removed as it is done by Registry.new().
            zira.modules.db.initialize(cr)
            zira.tools.config['load_language'] = lang
            cr.commit()

        registry = zira.modules.registry.Registry.new(db_name, demo, None, update_module=True)

        with closing(db.cursor()) as cr:
            env = zira.api.Environment(cr, SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = zira.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = zira.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = zira.tools.config['db_template']
...
def exp_duplicate_database(db_original_name, db_name):
    ...
    zira.sql_db.close_db(db_original_name)
    db = zira.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        cr.autocommit(True)     # avoid transaction block
    ...
    registry = zira.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = zira.api.Environment(cr, SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)

    from_fs = zira.tools.config.filestore(db_original_name)
    to_fs = zira.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    zira.modules.registry.Registry.delete(db_name)
    zira.sql_db.close_db(db_name)
    ...
    fs = zira.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'zira_dump': '1',
        'db_name': cr.dbname,
        'version': zira.release.version,
        'version_info': zira.release.version_info,
        'major_version': zira.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with zira.tools.osutil.tempdir() as dump_dir:
            filestore = zira.tools.config.filestore(db_name)
            ...
            zira.tools.exec_pg_command(*cmd)
            if stream:
                zira.tools.osutil.zip_dir(dump_dir, stream, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
            else:
                t=tempfile.TemporaryFile()
                zira.tools.osutil.zip_dir(dump_dir, t, include_dir=False, fnct_sort=lambda file_name: file_name != 'dump.sql')
                t.seek(0)
                return t
    else:
        cmd.insert(-1, '--format=c')
        stdin, stdout = zira.tools.exec_pg_command_pipe(*cmd)
        if stream:
...
def restore_db(db, dump_file, copy=False):
    ...
    with zira.tools.osutil.tempdir() as dump_dir:
        ...
        if zira.tools.exec_pg_command(pg_cmd, *pg_args):
            raise Exception("Couldn't restore database")

        registry = zira.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = zira.api.Environment(cr, SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    zira.modules.registry.Registry.delete(old_name)
    zira.sql_db.close_db(old_name)
    ...
    old_fs = zira.tools.config.filestore(old_name)
    new_fs = zira.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    zira.tools.config.set_admin_password(new_password)
    zira.tools.config.save()
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        zira.tools.config['update']['base'] = True
        zira.modules.registry.Registry.new(db, force_demo=False, update_module=True)
    return True
...
@zira.tools.mute_logger('zira.sql_db')
...
def list_dbs(force=False):
    if not zira.tools.config['list_db'] and not force:
        raise zira.exceptions.AccessDenied()

    if not zira.tools.config['dbfilter'] and zira.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Zira will not
        ...
        res = sorted(db.strip() for db in zira.tools.config['db_name'].split(','))
        return res
    ...
    with closing(db.cursor()) as cr:
        try:
            ...
            res = [zira.tools.ustr(name) for (name,) in cr.fetchall()]
        except Exception:
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Zira
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if zira.tools.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        zira.sql_db.close_db(database_name)
...
def exp_list_lang():
    return zira.tools.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(zira.tools.config['root_path'], 'addons/base/data/res_country_data.xml')).getroot()
