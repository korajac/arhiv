...
import zira
from zira.exceptions import UserError, ValidationError, QWebException
from zira.models import check_method_name
from zira.tools.translate import translate
...
def dispatch(method, params):
    ...
    registry = zira.registry(db).check_signaling()
    fn = globals()[method]
...
def check(f):
    ...
    def wrapper(___dbname, *args, **kwargs):
        ...
        def tr(src, ttype):
            ...
            try:
                cr = zira.sql_db.db_connect(dbname).cursor()
        ...
        while True:
            try:
                if zira.registry(dbname)._init and not zira.tools.config['test_enable']:
                    raise zira.exceptions.Warning('Currently, this database is not fully loaded and can not be used.')
...
def execute_cr(cr, uid, obj, method, *args, **kw):
    zira.api.Environment.reset()  # clean cache etc if we retry the same transaction
    recs = zira.api.Environment(cr, uid, {}).get(obj)
    if recs is None:
        raise UserError(_("Object %s doesn't exist") % obj)
    result = zira.api.call_kw(recs, method, args, kw)
...
def execute(db, uid, obj, method, *args, **kw):
    ...
    with zira.registry(db).cursor() as cr:
