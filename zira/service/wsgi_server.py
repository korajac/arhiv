...
# User code must use the exceptions defined in ``zira.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, zira.exceptions.UserError):
        fault = xmlrpclib.Fault(RPC_FAULT_CODE_WARNING, zira.tools.ustr(e.name))
    ...
    elif isinstance(e, zira.exceptions.DeferredException):
        info = e.traceback
        # Which one is the best ?
        formatted_info = "".join(traceback.format_exception(*info))
        #formatted_info = zira.tools.exception_to_unicode(e) + '\n' + info
...
def xmlrpc_handle_exception_string(e):
    if isinstance(e, zira.exceptions.UserError):
        ...
        info = e.traceback
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(zira.tools.ustr(e), formatted_info)
    #InternalError
    else:
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpclib.Fault(zira.tools.exception_to_unicode(e), formatted_info)
...
def _patch_xmlrpc_marshaller():
    ...
    # Historically, zira is sending binary as base64 string.
    ...
    def dump_bytes(marshaller, value, write):
        marshaller.dump_unicode(zira.tools.ustr(value), write)
...
def application_unproxied(environ, start_response):
    ...
    # web.session.KorajacSession.send() and at RPC dispatch in
    # zira.service.web_services.objects_proxy.dispatch().
    ...
    with zira.api.Environment.manage():
        result = zira.http.root(environ, start_response)
