# -*- coding: utf-8 -*-
"""
The module :mod:`zira.tests.common` provides unittest test cases and a few
...
from zira.models import BaseModel
from zira.osv.expression import normalize_domain, TRUE_LEAF, FALSE_LEAF
from zira.tools import pycompat
from zira.tools import single_email_re
from zira.tools.misc import find_in_path
...
import zira
import pprint
from zira import api
from zira.service import security



_logger = logging.getLogger(__name__)

# The zira library is supposed already configured.
ADDONS_PATH = zira.tools.config['addons_path']
HOST = '127.0.0.1'
PORT = zira.tools.config['http_port']
# Useless constant, tests are aware of the content of demo data
ADMIN_USER_ID = zira.SUPERUSER_ID


def get_db_name():
    db = zira.tools.config['db_name']
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most zira operations;
    """
    if not login:
...
# Main classes
# ------------------------------------------------------------
class ZiraSuite(unittest.suite.TestSuite):
...
class MetaCase(type):
    ...
    def __init__(cls, name, bases, attrs):
        ...
        if cls.__module__.startswith('zira.addons.'):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common Korajac-specific code.

    This class is abstract and expects self.registry, self.cr and self.uid to be
    initialized by subclasses.
    """

    longMessage = True      # more verbose error message by default: https://www.korajac.com/r/Vmh
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~zira.models.BaseModel`
        """
        ...
        if self.warm:
            ...
            with self.subTest(), patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/zira/addons/" in filename:
                        filename = filename.rsplit("/zira/addons/", 1)[1]
                    if count > expected:
...
class TransactionCase(BaseCase):
    ...
    def setUp(self):
        ...
        self.registry = zira.registry(get_db_name())
        ...
        #: :class:`~zira.api.Environment` for the current test case
        self.env = api.Environment(self.cr, zira.SUPERUSER_ID, {})
...
class SingleTransactionCase(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.registry = zira.registry(get_db_name())
        cls.addClassCleanup(cls.registry.clear_caches)

        cls.cr = cls.registry.cursor()
        cls.addClassCleanup(cls.cr.close)

        cls.env = api.Environment(cls.cr, zira.SUPERUSER_ID, {})
...
class ChromeBrowser():
    ...
    def __init__(self, logger):
        ...
        self.request_id = 0
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_zira')
    ...
    def _get_shotname(self, prefix, ext):
        ...
        base_file = os.path.splitext(zira.tools.config['logfile'])[0]
        name = '%s_%s_%s.%s' % (base_file, prefix, timestamp, ext)
        return name

    def take_screenshot(self, prefix='failed'):
        if not zira.tools.config['logfile']:
    ...
    def _save_screencast(self, prefix='failed'):
        ...
        if not zira.tools.config['logfile']:
            ...
            return None
        sdir = tempfile.mkdtemp(suffix='_chrome_zira_screencast')
        nb = 0
...
class HttpCase(TransactionCase):
    ...
    def setUp(self):
        ...
        self.session = zira.http.root.session_store.new()
        self.session_id = self.session.sid
        self.session.db = get_db_name()
        zira.http.root.session_store.save(self.session)
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('zira.service.http.request.'):
                join_retry_count = 10
    ...
    def authenticate(self, user, password):
        ...
        if user is None:
            if self.session:
                zira.http.root.session_store.delete(self.session)
        ...
        # KorajacSession.authenticate accesses the current request, which we
        ...
        zira.http.root.session_store.save(session)
        if self.browser:
...
class Form(object):
    ...
    * properly handle defaults & onchanges around x2many fields
    ...
    :class:`~zira.fields.Many2one` fields assign a singleton recordset::
    ...
    For :class:`~zira.fields.Many2many` fields, the field itself is a
    :class:`~zira.tests.common.M2MProxy` and can be altered by adding or
    removing records::
    ...
    Finally :class:`~zira.fields.One2many` are reified as
    :class:`~zira.tests.common.O2MProxy`.

    Because the :class:`~zira.fields.One2many` only exists through its
    parent, it is manipulated more directly by creating "sub-forms"
    with the :meth:`~zira.tests.common.O2MProxy.new` and
    :meth:`~zira.tests.common.O2MProxy.edit` methods. These would
    ...
    :type recordp: zira.models.Model
    ...
    :type view: int | str | zira.model.Model
    ...
    def _init_from_defaults(self, model):
        ...
        def cleanup(k, v):
            if fields[k]['type'] == 'one2many':
                ...
                return zira.fields.Datetime.to_string(v)
...
class O2MProxy(X2MProxy):
    ...
    def new(self):
        ...
        :class:`~zira.fields.One2many` record, properly initialised.
    ...
    def edit(self, index):
        ...
        :class:`~zira.fields.One2many` record.
...
def record_to_values(fields, record):
    ...
    for f, descr in fields.items():
        ...
        if descr['type'] == 'many2one':
            ...
            v = zira.fields.Datetime.to_string(v)
        elif descr['type'] == 'date' and isinstance(v, date):
            v = zira.fields.Date.to_string(v)
        r[f] = v
    return r
...
def _cleanup_from_default(type_, value):
    ...
    if type_ == 'one2many':
        ...
        return zira.fields.Datetime.to_string(value)
    elif type_ == 'date' and isinstance(value, date):
        return zira.fields.Date.to_string(value)
    return value
...
def tagged(*tags):
    ...
    By default, all Test classes from zira.tests.common have a test_tags
