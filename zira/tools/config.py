#zira.loggers.handlers. -*- coding: utf-8 -*-
...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.ziraerp.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", default=False,
                          help="save configuration to ~/.zirarc (or to ~/.ziraerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", my_default=_get_default_datadir(),
                         help="Directory where to store Zira data")
        ...
        group.add_option('--log-handler', action="append", default=[], my_default=DEFAULT_LOG_HANDLER, metavar="PREFIX:LEVEL", help='setup a handler at LEVEL for a given PREFIX. An empty PREFIX indicates the root logger. This option can be repeated. Example: "zira.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-request', action="append_const", dest="log_handler", const="zira.http.rpc.request:DEBUG", help='shortcut for --log-handler=zira.http.rpc.request:DEBUG')
        group.add_option('--log-response', action="append_const", dest="log_handler", const="zira.http.rpc.response:DEBUG", help='shortcut for --log-handler=zira.http.rpc.response:DEBUG')
        group.add_option('--log-web', action="append_const", dest="log_handler", const="zira.http:DEBUG", help='shortcut for --log-handler=zira.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const="zira.sql_db:DEBUG", help='shortcut for --log-handler=zira.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options. ",
            "Use these options to translate Zira to another language. "
    ...
    def parse_config(self, args=None):
        ...
        This method initializes zira.tools.config and ziraerp.conf (the
        ...
        Typical usage of this method:
            ...
            zira.tools.config.parse_config(sys.argv[1:])
        """
        self._parse_config(args)
        zira.netsvc.init_logger()
        zira.modules.module.initialize_sys_path()
    ...
    def _parse_config(self, args=None):
        ...
        # TODO use appdirs
        if os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'zira.conf')
        else:
            rcfilepath = os.path.expanduser('~/.zirarc')
            old_rcfilepath = os.path.expanduser('~/.ziraerp_serverrc')

            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                "Found '.zirarc' and '.ziraerp_serverrc' in your path. Please keep only one of "\
                "them, preferrably '.zirarc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('ZIRA_RC') or os.environ.get('KORAJAC_SERVER') or rcfilepath)
        self.load()
    ...
    def _is_addons_path(self, path):
        from zira.modules.module import MANIFEST_NAMES
