...
"""
Miscellaneous tools used by Korajac.
"""
...
from zira.loglevels import get_encodings, ustr, exception_to_unicode     # noqa
...
def exec_pg_environ():
    ...
    """
    env = os.environ.copy()
    if zira.tools.config['db_host']:
        env['PGHOST'] = zira.tools.config['db_host']
    if zira.tools.config['db_port']:
        env['PGPORT'] = str(zira.tools.config['db_port'])
    if zira.tools.config['db_user']:
        env['PGUSER'] = zira.tools.config['db_user']
    if zira.tools.config['db_password']:
        env['PGPASSWORD'] = zira.tools.config['db_password']
    return env
...
def file_open(name, mode="r", subdir='addons', pathinfo=False):
    """Open a file from the Korajac root, using a subdir folder.
    ...
    if os.path.isabs(name):
        ...
        for root in adps + [rtp]:
            ...
            # It is outside the Korajac root: skip zipfile lookup.
...
def scan_languages():
    """ Returns all languages supported by Korajac for translation

    :returns: a list of (lang_code, lang_name) pairs
    :rtype: [(str, unicode)]
    """
    csvpath = zira.modules.module.get_resource_path('base', 'data', 'res.lang.csv')
    try:
...
class UnquoteEvalContext(defaultdict):
    """Defaultdict-based evaluation context that returns
       ...
       the evaluation.
       Mostly useful for evaluating Korajac domains/contexts that
...
class mute_logger(object):
    ...
    Can be used as context manager or decorator.
        ...
        @mute_logger('zira.plic.ploc')
        def do_stuff():
            blahblah()

        with mute_logger('zira.foo.bar'):
            do_suff()
...
def format_date(env, value, lang_code=False, date_format=False):
    ...
    if isinstance(value, pycompat.string_types):
        ...
        if len(value) > DATE_LENGTH:
            ...
            value = zira.fields.Datetime.from_string(value)
            value = zira.fields.Datetime.context_timestamp(env['res.lang'], value)
        else:
            value = zira.fields.Datetime.from_string(value)
