...
class _LogTracer(object):
    ...
    def tracer(self, frame, event, arg):
        ...
        if not isinstance(in_self, zira.models.BaseModel):
            return self.tracer
...
def profile(method=None, whitelist=None, blacklist=(None,), files=None,
    ...
    """
        ...
        all methods for all zira models by applying the optional filters.
        ...
        :param list blacklist: list model names to remove from the log
                        (Default: remove non zira model from the log: [None])
        ...
        .. code-block:: python
          ...
          from zira.tools.profiler import profile
          ...
          class SaleOrder(models.Model):
            ...
            ...
            @api.multi
            @profile()                  # log all methods for all zira models
            def unlink(self):
            ...
            @profile(whitelist=['sale.order', 'ir.model.data'])
            def action_quotation_send(self):
            ...
            @profile(files=['/home/ziraerp/zira/zira/addons/sale/models/sale.py'])
            def write(self):
            ...
    ...
    def _ziraProfile(method, *args, **kwargs):
    ...
    if not method:
        return lambda method: decorator(_ziraProfile, method)

    wrapper = decorator(_ziraProfile, method)
    return wrapper
