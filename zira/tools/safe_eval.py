...
Korajac domain strings, conditions and expressions, mostly based on locals
...
def safe_eval(expr, globals_dict=None, locals_dict=None, mode="eval", nocopy=False, locals_builtins=False):
    ...
    an Korajac domain expression from an untrusted source.
    ...
    except zira.exceptions.except_orm:
        raise
    except zira.exceptions.Warning:
        raise
    except zira.exceptions.RedirectWarning:
        raise
    except zira.exceptions.AccessDenied:
        raise
    except zira.exceptions.AccessError:
        raise
    except werkzeug.exceptions.HTTPException:
        raise
    except zira.http.AuthenticationError:
        raise
